...
bool SceneManager::LoadWorldFromData(const FSceneData& sceneData, UWorld* targetWorld)
{
    ...
    for (const FActorSaveData& actorData : sceneData.Actors)
    {
        ...
        {
            //UE_LOG(ELogLevel::Error, TEXT("LoadSceneFromData: Could not find Actor Class '%s'. Skipping Actor '%s'."), *actorData.ActorClass, *actorData.ActorID);
            continue;
        }
        ...
        if (SpawnedActor == nullptr)
        {
            //UE_LOG(ELogLevel::Error, TEXT("LoadSceneFromData: Failed to spawn Actor '%s' of class '%s'."), *actorData.ActorID, *actorData.ActorClass);
            continue;
        }
        ...
        for (const FComponentSaveData& componentData : actorData.Components)
        {
            ...
            if (TargetComponent == nullptr)
            {
                 //UE_LOG(ELogLevel::Error, TEXT("LoadSceneFromData: Failed to create Component '%s' of class '%s' for Actor '%s'."), *componentData.ComponentID, *componentData.ComponentClass, *actorData.ActorID);
                continue;
            }
